...
MatrixXf EKF::move(MatrixXf x, MatrixXf u, float dt, float pitch)
{
	/* 動作予測
	 * x : 状態
	 * u : 制御
...
//	float theta = x.coeffRef(2,0) + u.coeffRef(1,0)*dt/2;
	float theta = x.coeffRef(2,0) + u.coeffRef(1,0)*dt;
...
MatrixXf EKF::jacobG(MatrixXf x, MatrixXf u, float dt, float pitch)
{
	/* ヤコビ行列
	 * x : 状態
	 * u : 制御
...
//	float theta = u.coeffRef(1,0)*dt/2 + x.coeffRef(2,0);
	float theta = u.coeffRef(1,0)*dt + x.coeffRef(2,0);
...
MatrixXf EKF::jacobV(MatrixXf x, MatrixXf u, float dt, float pitch)
{
	/* ヤコビ行列
	 * x : 状態
	 * u : 制御
...
/*	V << dt*cos(pitch)*cos(theta), (-v*dt*dt)*cos(pitch)*sin(theta)/2,
		 ...
		 0, dt;
*/	V << dt*cos(pitch)*cos(theta), (-v*dt*dt)*cos(pitch)*sin(theta),
		 dt*cos(pitch)*sin(theta), (v*dt*dt)*cos(pitch)*cos(theta),
		 0, dt;
