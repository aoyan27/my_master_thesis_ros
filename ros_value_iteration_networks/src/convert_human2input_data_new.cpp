...
class CvtHuman2Input
{
	private:
		...
		geometry_msgs::PoseStamped get_global_pose(geometry_msgs::PoseStamped local_pose);
...
geometry_msgs::PoseStamped
CvtHuman2Input::get_global_pose(geometry_msgs::PoseStamped local_pose)
{
	geometry_msgs::PoseStamped global_pose;
	global_pose.header.frame_id = "/map";
	global_pose.header.stamp = local_pose.header.stamp;
	try{
		tflistener_.waitForTransform("/map", "/velodyne", 
									 global_pose.header.stamp, 
									 ros::Duration(5.0));
		tflistener_.transformPose("/map", global_pose.header.stamp, local_pose, 
								  "/velodyne", global_pose);
	}
	catch(tf::TransformException &ex){
		ROS_ERROR("getPose() : can't transform_listener : %s\n", ex.what());
	}
	...
	if(num_human != 0){
		...
		for(size_t i=0; i<num_human; i++){
					...
					&& fabs(tmp_local.pose.position.y) <= CONSTRAINT_Y)
			{
				...
				human_pose_list.push_back(tmp_global);
