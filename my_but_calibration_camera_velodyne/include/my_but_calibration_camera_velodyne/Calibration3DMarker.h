...
#include <my_but_calibration_camera_velodyne/Calibration.h>
...
public:
	Calibration3DMarker(cv::Mat _frame_gray, cv::Mat _P, ::pcl::PointCloud<Velodyne::Point> _pc, float _circ_distance, float _radius);
...
protected:
	template<typename PointT>
		void remove_inliers(const ::pcl::PointCloud<PointT> &cloud_in, std::vector<int> inliers_indices, ::pcl::PointCloud<PointT> &cloud_out){
	...
	void generate_possible_points(::pcl::PointCloud< ::pcl::PointXYZ > &plane, ::pcl::PointCloud< ::pcl::PointXYZ >::Ptr detection_cloud, const std::vector< ::pcl::PointXYZ > &possible_centers, float radius, float tolerance);

	std::vector< ::pcl::PointXYZ > refine4centers(std::vector< ::pcl::PointXYZ > centers, ::pcl::PointCloud< ::pcl::PointXYZ >::Ptr detection_cloud);
