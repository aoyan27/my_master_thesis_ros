// point cloudからoccupancygrid mapを生成
// pathがしばらくひけないときにはexpandを小さくする (matchey)
#include <ros/ros.h>
...
void CombinedPC2Callback(sensor_msgs::PointCloud input){
...
int main(int argc,char** argv)
{
	...
	ros::init(argc, argv, "pc2local_map_added_recovery");
    ros::NodeHandle n;
	ros::Rate roop(20);
	...
	ros::Publisher pub_map = n.advertise<nav_msgs::OccupancyGrid>("/local_map_real", 1);
	ros::Publisher pub_map_expand 
		= n.advertise<nav_msgs::OccupancyGrid>("/local_map_real/expand", 1);
	...
	// ExpandMap ex_small_map(0.3, 0.1);
	ExpandMap ex_small_map(0.10, 0.1);
	// ExpandMap ex_map(0.4, 0.1);
	//tis
